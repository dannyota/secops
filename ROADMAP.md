...
### Wave 104 — SOAR-host bearer-token auth (`--soar-token`) — *reverted in v0.5.7*

Briefly shipped in v0.5.6: a third `Credentials` type (`auth.BearerToken`) and a global `--soar-token` flag to authenticate SOAR calls with a supplied bearer token instead of the AppKey. It was built on the premise that `run-action`'s 500 was an auth-identity problem — which Wave 105 disproved (the cause was an omitted alert group; AppKey works once the body is correct). With no surface that demonstrably needs a credential other than the AppKey, the feature was removed in v0.5.7 to avoid speculative auth surface area. The two credentials that remain are AppKey (SOAR plane) and ADC (chronicle plane).
...
### Wave 106 — `soar case run-action` exits non-zero on a FAULTED action *(built — offline-tested)*

The run-action API call returns 200 with the action's own outcome in the body, so a faulted action (server `status:FAULTED` / `resultName:is_failed` / `resultValue:false` / non-zero `resultCode`) previously printed `result: success (code 0)` and exited 0 — automation couldn't tell a real action failure from success. `renderActionResult` now reads the outcome from those fields, prints `result: FAULTED`, and returns an error so the command exits non-zero (both the human and `--json` paths); a genuine success still exits 0. Also in this release: a docs register pass normalized the catalog status vocabulary to `verified` and removed validation-method phrasing across `docs/design/*` (the public repo describes behavior, not how it was checked).

### Wave 107 — `soar settings grouping` reads/writes the full alert-grouping property bag *(built — offline-tested)*

`grouping get` surfaced only the legacy max-alerts value and reported the modern property bag as empty, because `moduleSettingPropertiesPage` decoded the list under a misspelled JSON key (`moduleSettingProperties`) while the v1alpha response returns it under `items` (and `moduleSettingsProperties`) — a one-character tag typo that dropped every property. Fixed the tag to the canonical `items`, so the bag now decodes its six levers: `TimeframeForGroupingInHours`, `MaxAGroupingForAlerts`, `OverflowTimeframeForGroupingInHours`, `OverflowMaxAGroupingForAlerts`, `GroupingAlgorithmType`, and `FallbackToSourceGroupingIdentifier` — the same properties the SOAR Settings → Alerts Grouping page edits. `ModuleSettingProperty` gains a `displayName`/`ShortName()` so `get` prints friendly names and `set --property <ShortName>=<value>` matches them, mapping back to the full resource name the `:batchUpdate` write expects. The Timeframe/Overflow/co-grouping knobs are now config-as-code, not UI-only.

### Wave 108 — Playbook execution ergonomics: scope auto-resolve + debug enum-dialect translation *(planned)*

Two follow-ons in the same family as Wave 105, scoped from an audit of the other case-bound execution surfaces:

- **`playbook attach` / `run` / `block-run` scope.** `AttacheWorkflowToCase` requires `alertGroupIdentifier` **or** `alertIdentifier` and returns a clean `400` ("Either alertGroupIdentifier or alertIdentifier must be provided") when both are omitted — it validates gracefully rather than NPE-ing like `run-action` did, so this is ergonomics, not a crash. The builders (`attachWorkflowBody`/`playbookBlockRunBody`) make both optional, so a run with neither round-trips to the 400. Apply the Wave 105 treatment: auto-resolve the alert group from the case when neither flag is given (require at least one otherwise), so the call just works or fails fast client-side.
- **`playbook debug` enum dialect.** `debug --file` posts the export-shape JSON straight to `RunPlaybookInDebug`, which `400`s ("Invalid arguments") on the string enums every exported playbook carries — `workflow.creationSource`, `workflow.executionScope`, `workflow.trigger.type`, `workflow.trigger.executionMode`. Translate those string tokens to the numeric enums the debug endpoint expects — mappings sourced from the swagger enum schema per the SDK's typed-enum convention — so `debug` is round-trippable from `export`/`pull`. Any residual server-side behavior past the 400 is tracked as a separate item.

### Wave 109 — Curated rules read suite: rule-sets, search/filter, view detail *(built — offline-tested)*

Built out full read support for Google-managed curated content, scoped from the console's Curated Detections view (which reads the chronicle-host v1alpha curated surface). The `CuratedRule` model was under-modeling the API response — it dropped `type`, `description`, the MITRE `tactics`/`techniques`, and crucially the parent **`curatedRuleSet`** (the rule→set membership). Enriched the struct (plus `MitreRef`, and `RuleSetID()`/`CategoryID()` helpers) and added three reads: **`curated rule-sets [--category]`** lists the curated rule SETS (`ListCuratedRuleSets` over the `-` wildcard); **`curated rules`** gains client-side `--search` (name/description), `--set` (enumerates a set's rules — every listed rule carries its `curatedRuleSet`), `--category`, `--tactic` (MITRE id or name), and `--severity` filters, with MITRE/precision columns; **`curated rule <id>`** shows one rule's full detail via `GetCuratedRule` (severity, type, precision, MITRE tactics/techniques, parent set, description). Curated rules are Google-managed, so the YARA-L source is not exposed by the API — the detail is metadata + MITRE + description. Per-set detection counts (`countAllCuratedRuleSetDetections`) for an inline blast-radius are noted for a follow-on. **Docs:** catalog-siem, usage.

### Wave 110 — Content Hub: reversible marketplace install/uninstall + browse *(built)*

A marketplace install had no reversible inverse in the CLI: `soar integration install` installs a marketplace pack (`marketplaceIntegrations:install`), but `soar integration uninstall` deletes only *custom* packs (`integrations.delete`) — so a marketplace install was one-way. Added the reversible pair under `soar marketplace`: **`install --identifier <id>`** and **`uninstall --identifier <id>`** (`marketplaceIntegrations:install`/`:uninstall`), both guarded (dry-run by default); `soar integration install` stays as the integration-scoped alias. Added **`soar marketplace browse`** — a one-shot Content Hub overview (integration + content-pack catalog totals and the installed-integration count, cross-referenced from the installed-pack list because the marketplace list does not populate `IsInstalled`). The install→uninstall round-trip is self-cleaning. Content-pack deploy/install (`contentHub/contentPacks:deployPlaybooks|deployConnectorInstances|deployTestCases`, `:delete`) stays SDK-only pending a captured deploy-body shape. Part of the **v0.6.0** milestone (with SIEM-search improvements). **Docs:** catalog-soar, usage.
