...
New to the tool? The **[task cookbook](docs/USAGE.md)** walks the full workflows
end to end (edit a rule, triage a case, reconcile a SOAR surface); unfamiliar
...
| `pull <target> [--filter EXPR] [--out DIR]` | Read-only pull of live SIEM config. Targets: `rules`, `reference_lists`, `data_tables`, `dashboards`, `curated`, `curated_rules`, `feeds`, `parsers`, `rule_exclusions`, `all`. |
| `push <target> [--dry-run \| --yes] [--prune]` | **Live deploy.** `rules-create` / `rules-update` / `rules-deploy` / `rules-disable` + engine-reconciled surfaces. Defaults to `--dry-run`. |
| `query udm <filter> [--hours N] [--from/--to TS] [--limit N]` | Ad-hoc UDM event search. |
| `rules detections/errors/alerts <id>` · `rules retrohunt …` | Read a rule's detections / errors / alerts; manage retrohunts. |
...
| `soar case <verb> …` | Per-case triage (list / get / assign / rename / stage / tag / close / merge, …). The reliable case path. |
| `soar marketplace list/get/contentpacks` | Browse the SOAR Content Hub (integration + content packs). |
| `soar integration` · `soar settings` | Manage integration instances / connector defs; singleton case-routing policies. |
...
Three standalone, importable clients (pure API — no file I/O), split by surface +
credential; constructing one never touches the network (auth resolves lazily):
...
Full guide — all three clients, split auth, error handling, pagination, and
chronicle + soar examples — in [`docs/SDK.md`](docs/SDK.md).

## Documentation

| Doc | What it is |
|---|---|
| [`docs/USAGE.md`](docs/USAGE.md) | **Task cookbook** — copy-paste end-to-end workflows (start here for *how do I…*) |
| [`docs/GLOSSARY.md`](docs/GLOSSARY.md) | Plain-language definitions of every term used in the docs/CLI |
| [`docs/SDK.md`](docs/SDK.md) | Using the Go SDK — the three clients, auth, errors, examples |
...
| [`docs/SURFACES.md`](docs/SURFACES.md) | The API surface map — which API does what, by plane, with gaps |
