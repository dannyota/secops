...
func sampleRuleExclusion() chronicle.RuleExclusion {
	return chronicle.RuleExclusion{
		...
		Query:       `principal.hostname = "scanner-01"`,
...
func TestRuleExclusionRoundTrip(t *testing.T) {
	live, err := ruleExclusionObject(sampleRuleExclusion())
	if err != nil {
		t.Fatalf("build: %v", err)
	}
	cs := string(live.Canonical)
	if bytes.Contains(live.Canonical, []byte("etag-xyz")) || bytes.Contains(live.Canonical, []byte("fr_123")) {
