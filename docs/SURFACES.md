...
| SLA · networks · block-lists · ontology · remote agents | 🔨 modern twins **read-confirmed live on siemplify-soar v1alpha** (`slaDefinitions`, `soarNetworks`, `entitiesBlocklists`, `ontologyRecords` (2.3 MB), `remoteAgents`); full CRUD documented (+ `export`/`import`, `soarNetworks:deleteAll`, ontology `visualFamilies`/`mappingRules`, remote-agent install/upgrade verbs) | SDK not built — reconcile/raw runs on legacy; flip per surface. `slaDefinitions` uses **string** enums (not the legacy ints) |
