...
| schema discovery — `feedSourceTypeSchemas` · `logTypeSchemas` · `logTypeSetting` | imperative (read) | ✅ chronicle · v1alpha | — | SDK (`chronicle/schemas.go`): `ListFeedSourceTypeSchemas` (the available feed source types), `ListLogTypeSchemas(sourceType)` (accepted log types + required detail fields per source type), and `GetLogTypeSetting` (per-log-type ingestion config). The reference for validating feed YAML before a deploy. Read-only (upstream-defined). Rides the feeds family — project ID form, v1alpha default. Read-validated live (`TestLiveSchemaDiscoveryRead`). Per-log-type GET (`logTypes.get`) is not wired — it 404s "Method not found" here; log types are enumerated via `ListLogTypes`. Wire into feed validation |
