...
| governance — `riskConfig` · `dataAccessLabels`/`Scopes` | imperative | 🔨 chronicle · v1 | — | SDK built (`chronicle/rbac.go`, `rbacAPIVersion`; all three versions answer → pinned v1). **dataAccessLabels CRUD live-validated** (Wave 10, `TestLiveDataAccessLabelWriteSmoke` — self-cleaning create→get→delete on an inert unbound throwaway). The surface is quirky: **create can return an error yet still persist**, **create→list lags**, deleted ids **tombstone**, and the create body's `displayName` is ignored (server uses the id) — so it is operated **imperatively with unique self-identifying ids + delete-by-exact-id**, not via the reconcile engine (list lag breaks diffing). `riskConfig` is the singleton `{instance}/riskConfig` (GET live-validated; `UpdateRiskConfig` built, write pending). Scopes CRUD built, write pending. No CLI wired yet |
