...
| schema discovery — `feedSourceTypeSchemas` · `logTypeSchemas` · `logTypes` · `logTypeSetting` | imperative (read) | 🔨 chronicle · v1alpha | — | SDK (`chronicle/schemas.go`): `ListFeedSourceTypeSchemas` (the available feed source types), `ListLogTypeSchemas(sourceType)` (accepted log types + required detail fields per source type), `GetLogType`/`GetLogTypeSetting`. The reference for validating feed YAML before a deploy. Read-only (upstream-defined). Rides the feeds family — project ID form, v1alpha default. Read smoke `TestLiveSchemaDiscoveryRead`. Wire into feed validation |
