...
| `--soar-token <v>` | Advanced override: authenticate SOAR calls with a SOAR-host bearer token (e.g. a session JWT from the web console) sent as `Authorization: Bearer`, instead of the AppKey — a call then runs under the supplied identity. Accepts a literal, `env:VAR`, or `@file`; also `$SECOPS_SOAR_TOKEN`. See [configure.md](configure.md#soar-auth-session-bearer-token-advanced-override). |
...
| `soar case run-action --id N --action <name> --instance <uuid> [--integration <id>]` | Execute an integration action on a case (ad-hoc — any installed action). For a marketplace integration's action pass `--integration <id>` (e.g. GoogleChronicle) so the action is sent qualified as `<id>_<action>` — a bare name does not resolve; built-in Scripts actions (HTTP_Ping) need no `--integration`. With `--integration`, `--param` is pre-flight-validated against the action's schema (missing mandatory param → clean abort; `--skip-validate` bypasses). Script params via `--param key=value` (secrets via `env:VAR`). Returns the action result (resultCode, message); `--json` emits the full payload. (`--case-id` stays as a hidden alias of `--id`.) |
