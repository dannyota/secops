...
| `rule_exclusions` (findings refinements) | typed: display_name + type + UDM query + deployment | **done** (engine, `pull`/`push rule_exclusions`): Create + Update (PATCH/updateMask); NoDelete (no delete API), NoEtag; deployment state (`enabled`, `archived`) round-trips in YAML and is patched through the deployment subresource. Guarded `rule_exclusions deploy <id> --enable|--disable|--archive` handles one-off toggles. Read + write live-validated (create→update→archive); no hard delete exists — **archive** (deployment `archived=true`) is the documented teardown. This is the "exclusions" the curated row points to |
