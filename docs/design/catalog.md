...
| `cases run-action` | imperative | ✅ siemplify · v1alpha | ✅ | Runs any installed integration action on a case; guarded. |
...
| `cases task/chat/context/evidence <verb>` | imperative | — | ✅ | Sub-groups with guarded mutations: `task` (4 cmds, 3 guarded — checklist tasks), `chat` (3 cmds, 1 guarded — case-wall chat), `context` (2 cmds, 1 guarded — key-value context), `evidence` (2 cmds, 1 guarded — file attachments). |
| `cases aging/wall/workload/overview/custom-fields/stats/counts/values/soar-id` | operational read | — | ✅ | Read-only case helpers: aging report, case wall, analyst workload, per-case overview, custom-field values, queue stats, priority counts, valid enum values, SIEM-uuid→SOAR-id resolution. |
| `cases playbook-errors <id>` | operational read | — | ✅ | Walk all alerts' playbook executions and show faulted steps with full error messages; recurses into nested BLOCK playbooks. |
...
| `playbooks get` / `list` / `lint` / `health` / `diff` / `duplicate` / `move` / `categories` | operational read + guarded mutation | ✅ duplicate · categories — siemplify · v1alpha | ✅ | W118+W120; authoring inspection + folder management. `lint` detects dangling step relations (orphaned `stepsRelations` entries referencing deleted steps — causes console 500 on edit), broken block refs, missing integrations, placeholder-in-JSON, whitespace-poisoned triggers. `duplicate` uses modern v1alpha primary with `--folder`/`--env`, legacy+export→save fallback. `categories` (alias `folders`): list/create/rename/delete. `move`: relocate playbook to a category. |
...
| `integrations import` | guarded write | 🔨 siemplify · v1alpha | 🔨 | Import a packaged custom integration ZIP (scaffold → package → import); creates integration + action/job definitions with Python scripts; staging by default. |
...
| `mcp serve` / `mcp install` | offline utility | — | — | W139–140; MCP server over stdio — auto-generates tools from cobra tree + serves docs/tips as resources. Focus-first design: `focus <group>` loads typed tools (validated schemas), `usage <cmd>` auto-loads a single tool, `run` is the unvalidated escape hatch. Tool annotations (`readOnlyHint`/`destructiveHint`) from `commandKind`; `run` nudges toward `focus` when a typed tool exists. Progressive disclosure via `listChanged`. Results over 64 KiB spill to a temp file, returned as a JSON pointer with head preview (W144). `install` registers in `.mcp.json`. |
...
| global `--output table\|json\|csv` | output contract | — | — | W127; `--output json` ≡ `--json` everywhere; table/csv on the format-aware commands (`search udm`, `mitre`, `rules health`), where a local `--format` overrides it. |
