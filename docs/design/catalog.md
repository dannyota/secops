...
| `soar playbook` operational helpers | operational read + guarded execution | — | 🔨 | Wave 39. Read helpers: list/validate/components integrations/actions/jobs/connectors/test-cases/debug-step-data/simulation-enrichment/summary/results/result/python-logs. Guarded execution: `run`, `debug`, `rerun`, `rerun-block` all dry-run by default and require explicit case/test-case/workflow selectors before SecOps executes anything. Human output summarizes counts/status/presence only; `--json` returns raw SecOps payloads for scripts. Live write not run by default because playbooks can create cases, tasks, alerts, and external side effects |
| `soar job` operational helpers | operational read + guarded execution | — | 🔨 | Wave 39. `soar job list`, `soar job template list`, and `soar job instance list` fetch legacy job/runtime catalogs without printing script bodies. `soar job run` and `soar job instance run` resolve one explicit id/uniqueIdentifier/name, preview the target, and require `--yes` before SecOps executes it. `--json` returns dry-run/apply metadata and raw response when applied. Live execution is not run by default because jobs can create cases, tasks, alerts, and external side effects |
...
| `soar build-playbook` / `soar playbook mold extract` | offline utility | — | — | composes a save-ready playbook JSON file from a full exported base playbook. Sets `trigger.cronSchedule` and can replace named placeholder steps with exported, already-wired integration-action step molds while preserving the base step graph identity. `soar playbook mold extract` extracts one exported action step into that reusable mold format. Offline-only; SOAR still validates the final body through `soar push playbook --dry-run` / save. Built + offline-tested |
| `soar package-integration` | offline utility | — | — | packages an already-shaped SOAR custom integration directory into a deterministic ZIP for IDE import. Refuses symlinks, skips VCS/OS junk, requires at least one JSON definition/manifest, and warns when no `.py` files are present. Built + offline-tested; no API call |
