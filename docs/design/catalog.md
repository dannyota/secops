...
| `soar build-playbook` | offline utility | — | — | composes a save-ready playbook JSON file from a full exported base playbook. Sets `trigger.cronSchedule` and can replace named placeholder steps with exported, already-wired integration-action step molds while preserving the base step graph identity. Offline-only; SOAR still validates the final body through `soar push playbook --dry-run` / save. Built + offline-tested |
