...
| **raw logs** | operational (read) | ✅ chronicle · v1alpha (`query udm … --raw`) | — | recent **FULL** raw (ingested) log lines for parser development, via `query udm '<filter>' --raw` (no dedicated command — it rides the UDM search). The raw-log-search (`:searchRawLogs`) log-type filter is **ignored server-side**, so filtering is done with the regular UDM query; each matched event's `udm.metadata.id` is the raw-log id, fetched in batches of 25 via `legacyFindRawLogs?ids=` (GET) → COMPLETE bytes (`logBytes`, base64-decoded), one line per log → pipe into `parsers run --logs -`. A log type whose parser is missing/broken normalizes to **GENERIC_EVENT** (still `parsed = true`), so `metadata.log_type = "<TYPE>" AND metadata.event_type = "GENERIC_EVENT"` returns exactly the logs a parser fix targets. With `--raw`, `--limit` defaults to 100 (one raw fetch per event). Live read-validated against KONG_GATEWAY (`TestLiveFetchRawLogLines`). `chronicle/log_search.go` |
