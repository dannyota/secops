...
|---|---|---|---|---|
| `rules` | bespoke | ✅ chronicle · v1alpha | — | YARA-L source + deployment state machine (two resources), not a single canonical body. `push rules-create` (initial `--enabled`, `--alerting`, `--run-frequency`) · `rules-update` (etag-guarded text update) · `rules-deploy` (reconcile enabled/alerting/frequency; archived rules report non-deployable) · `rules-disable`. Pull mirrors `deployment.archived`; operational `rules detections/errors/alerts <id>` + `rules retrohunt list/get/create`; `rules errors` decodes both string and structured execution-error payloads. `RunTestRule` (dry-run vs historical data) + `ArchiveRule`. Read live-validated; lifecycle write smoke `TestLiveRulesLifecycleWriteSmoke` (create→update→deploy→archive→delete, self-cleaning) |
