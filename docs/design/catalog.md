...
| `soar job` operational helpers | operational read + guarded execution | — | 🔨 | Wave 39. `soar job list` and `soar job instance list` fetch legacy job/runtime catalogs without printing script bodies. `soar job run` and `soar job instance run` resolve one explicit id/uniqueIdentifier/name, preview the target, and require `--yes` before SecOps executes it. `--json` returns dry-run/apply metadata and raw response when applied. Live execution is not run by default because jobs can create cases, tasks, alerts, and external side effects |
