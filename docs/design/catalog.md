...
| **raw logs** | operational (read) | ✅ chronicle · v1alpha (`logs raw <LOG_TYPE>`) | — | recent **FULL unparsed** log lines for parser development. Two-step: `:searchRawLogs` collects raw-log ids, then `legacyFindRawLogs?ids=` (GET) downloads the COMPLETE bytes — the search match itself only carries an 80-char preview `snippet`, and `logBytes` is base64. Prints one line per log → pipe into `parsers run --logs -`. `--unparsed` appends `parsed = false` (logs that aren't normalizing — the broken-parser case); `--since` / `--limit` / `--query`. Live read-validated (`TestLiveFetchRawLogLines`; a returned line >80 chars proves the full-bytes path, not the snippet). `chronicle/log_search.go` |
