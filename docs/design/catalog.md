...
| `playbooks` | reconcile (bespoke) | — | ✅ | uuid rotates → key on **name**; whole-body save; SavePlaybook update-by-name verified. Local playbook loads and singular `soar push playbook --dry-run` validate the save-time name charset (`[A-Za-z0-9 _-]`) before any API call. (Playbooks/workflows exist **only** on Legacy — no New twin) |
...
| `connector-allowlist` | reconcile (derived) | — | 🔨 | alert allow-list view over connector instances. `pull` writes sanitized files under `soar/connector_allowlist/` with context plus `allowList`; drift compares only `allowList`. `push` is update-only: it fresh-reads the full connector body, replaces `allowList`, and calls `SaveConnector`, preserving parameters/secrets and refusing non-empty allow-lists on connectors that do not support them. Built + offline-tested; live write-smoke pending |
