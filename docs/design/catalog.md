...
| `mcp serve` / `mcp install` | offline utility | — | — | W139–140; MCP server over stdio — auto-generates 361 tools from cobra tree + serves docs/tips as resources. `install` registers in `.claude/settings.json`. No external deps; zero-config once `secopsctl config` is done. Replaces the former `skill` / `skill install` commands. |
