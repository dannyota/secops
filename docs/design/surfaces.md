...
| Family | Lane | Status | Gaps |
|---|---|---|---|
| dashboards (native) | reconcile + imperative | ✅ | `definition.charts[].dashboardChart` is a scalar ref — the YARA-L lives in separate `dashboardCharts`→`dashboardQueries`. Default pull is reference-only; **`pull dashboards --with-charts` derefs charts inline** (query-bearing) and `push`/`drift` reconcile them (`:addChart`/`:editChart`; W72, live-validated). Chart-query authoring also via SDK `AddChart`/`EditChart` + CLI `dashboards add-chart`/`edit-chart`/`remove-chart`/`charts` (W70/W71). Open edge: layout/reorder/removal still applied via UI / `remove-chart`, not `push` |
