...
| Family | Lane | Status | Gaps |
|---|---|---|---|
| dashboards (native) | reconcile + imperative | ✅ | reconcile is **reference-only** (`definition.charts[].dashboardChart` is a scalar resource-name ref — the YARA-L lives in separate `dashboardCharts`→`dashboardQueries`); chart-query authoring via SDK `AddChart`/`EditChart` (`:addChart`/`:editChart`, W70) surfaced as CLI `dashboards add-chart`/`edit-chart`/`remove-chart`/`charts` (W71, live-validated). Lossless deref-on-pull round-trip ⬜ |
