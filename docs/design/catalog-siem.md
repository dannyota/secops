...
**`query stats` (W81, live-validated):** a plain event search (`query udm`) 400s on a `match:`/`outcome:` aggregation; `query stats '<aggregation YARA-L>'` posts it to the stats path of `:udmSearch` (`chronicle.GetStats`) and prints the computed columns and rows (`--json` for raw output) — the CLI way to validate the exact YARA-L a dashboard chart uses before authoring. The `match:` section takes a field reference (`target.hostname`), not `$var = field`. Flags parse in any position (interspersed).
