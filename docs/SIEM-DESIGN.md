...
| `rule_exclusions` (findings refinements) | typed: display_name + type + UDM query | **done** (engine, `pull`/`push rule_exclusions`): Create + Update (PATCH/updateMask); NoDelete (no delete API), NoEtag; deployment toggle out of the diff basis. Read live-validated; live create not exercised (no delete to self-clean a throwaway). This is the "exclusions" the curated row points to |
